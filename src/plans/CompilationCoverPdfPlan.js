...
class CompilationCoverPdfPlan {
    ...
    return new Promise((resolve, reject) => {
      connection((db) => {
        ...
        collection.findOne({ _id: this.compilationId }, (err, doc) => { // eslint-disable-line consistent-return
          ...
          this.log('tried to find compilation');
  ...
  buildPdf() {
    ...
    return pdfHelper.buildPdf(html, 'cover', cover, config.coverOptions);
  }
  ...
  savePdfResults(pdfResults) {
    return new Promise((resolve, reject) => {
      connection((db) => {
        this.cover.pdf = pdfResults;

        const collection = db.collection('compilation');
        collection.update(
